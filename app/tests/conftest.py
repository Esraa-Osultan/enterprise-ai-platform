...
def other_auth_headers(client):
    """A second, distinct user -- used to verify one user can't see or
    touch another user's documents."""
    client.post(
        "/auth/register",
        json={"username": "engineer2", "email": "eng2@valeo.com", "password": "secret123"},
    )
    response = client.post(
        "/auth/login",
        data={"username": "engineer2", "password": "secret123"},
    )
    token = response.json()["access_token"]
    return {"Authorization": f"Bearer {token}"}
